...
struct bits {
    ...
    constexpr bits(const T& v) noexcept : bits(1 << static_cast<value_type>(v)) {}
    ...
    template<class... Args>
    constexpr bits(const T& v, Args&&... args) noexcept : v(build(v, args...)) {}
...
private:
    constexpr bits(const value_type& v) noexcept : v(v) {}
