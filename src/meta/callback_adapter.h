...
class UniqueCallbackAdapter final {
    ...
    using ParameterFunction = void NTAPI(Parameter);
    using Deleter = ParameterFunction *;
    using Invoker = ParameterFunction *;
    static void NTAPI noopParameterFunction(Parameter) {}
    ...
    struct Helper {
        ...
        static auto param(Functor &&functor) -> Parameter {
            if constexpr (is_small) {
                ...
                return std::bit_cast<Parameter>(std::make_unique<Functor>(std::forward<Functor>(functor)).release());
        }
        static void NTAPI deleter(Parameter param) {
            if constexpr (is_small) {
        ...
        static void NTAPI invoker(Parameter param) {
            if constexpr (is_small) {
                auto &functor = *std::launder(std::bit_cast<Functor *>(&param));
                functor();
                functor.~Functor();
