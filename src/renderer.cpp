...
namespace renderer {
	...
	ComPtr<IDXGISwapChain1> createSwapChain(const ComPtr<IDXGIFactory2>& factory, const ComPtr<ID3D11Device>& device, HWND window) {
		...
		auto result = factory->CreateSwapChainForHwnd(device.Get(), window, &swap_chain_desription, nullptr, nullptr, &swap_chain);
	...
	ComPtr<ID3D11Texture2D> getTextureFromHandle(const ComPtr<ID3D11Device>& device, HANDLE handle) {
		...
		auto result = device->OpenSharedResource(handle, __uuidof(ID3D11Texture2D), &output);
	...
	ComPtr<ID3D11RenderTargetView> renderToTexture(const ComPtr<ID3D11Device>& device, const ComPtr<ID3D11Texture2D>& texture) {
		...
		auto result = device->CreateRenderTargetView(
			texture.Get(), render_target_description, &output);
