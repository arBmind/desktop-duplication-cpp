...
WaitableTimer::WaitableTimer(Config config)
    ...
    m_handle.reset(::CreateWaitableTimerExW(
        securityAttributes, m_timerName.data(), flags, config.desiredAccess));
}

void WaitableTimer::cancel() { ::CancelWaitableTimer(m_handle.get()); }

bool WaitableTimer::setImpl(
    WaitableTimer::SetArgs args, PTIMERAPCROUTINE callback, LPVOID callbackPtr) {
    auto queueTime = LARGE_INTEGER{};
    std::visit(
        [&]<class T>(const T &v) {
            if constexpr (std::is_same_v<T, TimePoint>) {
                queueTime.QuadPart =
                    std::chrono::duration_cast<HundredNanoSeconds>(v.time_since_epoch()).count() +
                    116444736000000000;
            }
    ...
    auto success = ::SetWaitableTimerEx(
        handle(), &queueTime, period, callback, callbackPtr, nullptr, tolerableDelay);
    return success != 0;
}
