...
void pointer_updater::update(const captured_update & data, const frame_data & context) {
	...
	if (pointer_desktop_m == context.output_desc.DesktopCoordinates
		|| (info.PointerPosition.Visible
			&& (!pointer_m.visible || info.LastMouseUpdateTime.QuadPart > pointer_m.last_update))) {
		pointer_desktop_m = context.output_desc.DesktopCoordinates;
		pointer_m.last_update = info.LastMouseUpdateTime.QuadPart;
